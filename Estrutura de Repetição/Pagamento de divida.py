...
print('Valor da Divida(R$)  Parcelas    Juros(%)   valor da parcela(R$)')
for parcela in range(0, 13, 3): # Os valores iterados representam as parcelas.
    ...
    valor_parcela = float() # A cada laço "valor_parcela" é zerada.
    if parcela == 3:
        juros = (valor_divida * 10) / 100
        for i in range(3):
            valor_parcela = (valor_parcela + valor_divida + juros)
        valor_parcela = (valor_parcela / 3)
    ...
    if parcela == 12:
        ...
        for i in range(12):
            valor_parcela = (valor_parcela + valor_divida + juros)
        valor_parcela = (valor_parcela / 12)
        print(f'{valor_divida:.2f}'.rjust(13)+f'{parcela}'.rjust(12)+f'{juros:.1f}'.rjust(15)+f'{valor_parcela:.2f}'.rjust(17)+'\n')
